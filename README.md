...
`live-transcribe` の文字起こしを追いかけて、Claude が議事録を書き続けます。
決まったこと・ネクストアクション(担当と期日)・未決の論点・漏れているかもしれない観点が、会議中に横目で読める短さで画面に出ます。
Claude には [Claude Agent SDK](https://code.claude.com/docs/en/agent-sdk) 経由で、毎回「今の議事録」と「前回の更新以降の発言」だけを渡します。発言の全文を毎回送り直すことはありません。

```sh
# 準備: Claude Code にログイン済みであること(claude コマンドが動くこと)。uv があれば依存は自動で入る
./live-transcribe            # 端末 1
./live-minutes               # 端末 2。data/ の最新の文字起こしを追いかける(引数でファイル指定も可)
```
